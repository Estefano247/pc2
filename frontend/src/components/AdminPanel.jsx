...
export default function AdminPanel({ user }) {
  ...
  const [libro, setLibro] = useState({ titulo: "", isbn: "", precio: "", autor_id: "", resumen: "", stock: 0 });
  ...
  const guardarLibro = async (e) => {
    ...
    const { data, error } = await supabase
      .from("libros")
      .insert([{ titulo: libro.titulo, isbn: libro.isbn, precio: parseFloat(libro.precio) || 0, autor_id: libro.autor_id || null, resumen: libro.resumen }])
      .select();
      ...
      <div className="admin-card">
        ...
        <form onSubmit={guardarLibro}>
          ...
          <button type="submit" disabled={loadingLibro}
            ...
            {loadingLibro ? "Publicando..." : "Publicar Libro"}
          </button>
        </form>
      </div>
